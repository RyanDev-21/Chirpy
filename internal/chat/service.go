...
type ChatService interface {
	...
	fetchMessage(ctx context.Context,userID ,chatID uuid.UUID)error //the chatID will be otherUserID if private
...
var ErrNOtFoundClient = errors.New("not found client")
...
func (s *chatService) sendMessage(ctx context.Context, userID uuid.UUID, payload *chatmodel.Message) error {
	...
	switch payload.Type {
	case "private":
		err:=handlePrivateMsg(userID, toID, payload, s.rediscache) // update the cache
		if err !=nil{
			return err
		}
	...
	case "public":
		err:=handlePublicMsg(userID, payload, s.rediscache)
		if err !=nil{
			return err
		}
...
func (s *chatService)fetchMessage(ctx context.Context,userID ,chatID uuid.UUID)error{
