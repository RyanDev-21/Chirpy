...
const (
	...
	REQCONTEXTKEY  ContextKey = "reqlog_id"
	PAYLOADCONTEXT ContextKey = "paylodContext"
)
...
func AuthMiddleWare(next http.HandlerFunc, secret string, logger *slog.Logger) http.HandlerFunc {
...
func getReqContextKey(context context.Context) (uuid.UUID, error) {
	payload, ok := context.Value(PAYLOADCONTEXT).(payload)
	if !ok {
		return uuid.Nil, errors.New("failed to get the request key")
	}
	return payload.reqContext, nil
}
